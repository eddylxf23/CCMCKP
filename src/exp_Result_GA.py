...
from src.functions.func_GA import GeneticAlgorithm
...
def param_test(Aw, fi, param,T_m, CL, MC_sample_size, Evaluation, _maxiter, file_1,file_2, seed, quick_check_list, data_folder, sign, times):
    print(f"GA_{sign}_{int(param[0])}_{int(param[1])}_{int(param[2])}_{T_m} 第{times} 开始！")
    start_time = time.time()
    Solution_LS = GeneticAlgorithm(Evaluation,T_m, fi, Aw, CL, param, _maxiter, MC_sample_size, quick_check_list)
    ...
    print(f"GA_{sign}_{int(param[0])}_{int(param[1])}_{int(param[2])}_{T_m} 第{times} 已完成！")

def create_csv(file_1, file_2):  
    path1 = my_file.real_path_of(f'result/GA/{file_1}')
    with open(path1,'w',newline='') as f:
        csv_write = csv.writer(f)
        csv_head = ['Times','Solution','Cost','Confidence Level','Evaluation Times','Real Confidence Level','Running Time']
        csv_write.writerow(csv_head)

    path2 = my_file.real_path_of(f'result/GA/{file_2}')
    ...
    for s in sign:
        ...
        for i in range(len(Instance_folder)):
            ...
            for j in range(len(Tmax[i])):
                ...
                file_1 = f'GA_{s}_{int(param[0])}_{int(param[1])}_{int(param[2])}_{T_m}.csv'
                file_2 = f'GA_{s}_{int(param[0])}_{int(param[1])}_{int(param[2])}_{T_m}_temp.csv'
            ...
            while len(process_list)>=40:
                for p in process_list:
                    if not p.is_alive():
                        p.join()
                        process_list.remove(p)
                if len(process_list)>=40:
                    time.sleep(1)
