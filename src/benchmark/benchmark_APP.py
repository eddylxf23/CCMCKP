...
生成每个因子的分布，并进行采样
    1) 模拟采样:
        采样数很小,由 sample_size 控制
    2) 真实蒙特卡洛采样:
        对分布进行直接采样，采样数很大，由 big_sample_size 控制。
        采样用来估计真实概率分布。
    3) 共有四种分布,每个因子随机挑选分布进行生成。
    4) 对单个item来说, 生成方式:
        a. 随机生成分布;
        b. 按照 0.999, 0.000999, 0.000000999, 0.000000001的概率, 在(0,10],(10,20],(20,30],(30,40]随机生成截断分布并采样
        c. 进行L次, 作为样本
采样完成后保存
